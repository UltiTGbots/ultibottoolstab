...
async function submitWithdrawToIndexer(params: any): Promise<string> {
    try {
        logger.debug('Submitting withdrawal to relayer with params:', {
        ...
        if (!response.ok) {
            ...
            logger.error('Relayer rejected withdrawal:', errorMessage);
            logger.error('Withdrawal params sent:', {
                extAmount: params.extAmount,
                fee: params.fee,
                recipient: params.recipient
            });
...
export async function withdraw({ recipient, lightWasm, storage, publicKey, connection, amount_in_lamports, encryptionService, keyBasePath }: WithdrawParams) {
    const originalAmount = amount_in_lamports;
    ...
    logger.debug(`Withdrawal fee calculation: ${(originalAmount / LAMPORTS_PER_SOL).toFixed(6)} SOL * fee_rate + 1 SOL * rent_fee = ${(fee_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL`);
    amount_in_lamports -= fee_in_lamports
    
    // Check if amount is still positive after fees
    if (amount_in_lamports <= 0) {
        throw new Error(`Withdrawal amount too small. After fees (${(fee_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL), the amount would be ${(amount_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL. Minimum withdrawal is ~0.001 SOL.`);
    }
    ...
    logger.info(`Total input amount: ${(totalInputAmount.toNumber() / LAMPORTS_PER_SOL).toFixed(6)} SOL`);
    logger.info(`Requested withdrawal: ${(originalAmount / LAMPORTS_PER_SOL).toFixed(6)} SOL`);
    logger.info(`Fees: ${(fee_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL`);
    
    if (totalInputAmount.toNumber() === 0) {
        throw new Error('No balance available. Please make a deposit first.');
    }
    
    if (totalInputAmount.lt(new BN(amount_in_lamports + fee_in_lamports))) {
        logger.warn(`Insufficient UTXOs. Total input: ${(totalInputAmount.toNumber() / LAMPORTS_PER_SOL).toFixed(6)} SOL, Required: ${((amount_in_lamports + fee_in_lamports) / LAMPORTS_PER_SOL).toFixed(6)} SOL`);
        isPartial = true
        const originalAmountBeforePartial = amount_in_lamports;
        amount_in_lamports = totalInputAmount.toNumber()
        amount_in_lamports -= fee_in_lamports
        
        // Check if after fees, we still have a positive amount
        if (amount_in_lamports <= 0) {
            const maxWithdrawable = Math.max(0, (totalInputAmount.toNumber() - fee_in_lamports) / LAMPORTS_PER_SOL);
            throw new Error(
                `Insufficient balance. Your 2 largest UTXOs total ${(totalInputAmount.toNumber() / LAMPORTS_PER_SOL).toFixed(6)} SOL, ` +
                `but fees require ${(fee_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL. ` +
                `Maximum withdrawable: ${maxWithdrawable.toFixed(6)} SOL. ` +
                `Try making another deposit to consolidate UTXOs into larger amounts.`
            );
        }
        
        logger.info(`Partial withdrawal: Requested ${(originalAmountBeforePartial / LAMPORTS_PER_SOL).toFixed(6)} SOL, but only ${(amount_in_lamports / LAMPORTS_PER_SOL).toFixed(6)} SOL available after fees`);
    }
